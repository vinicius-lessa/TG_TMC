...
function criarPedido($cod_cliente, $valor_pedido, $cod_carrinho, $conn)
{
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ids", $cod_cliente, $valor_pedido, $cod_carrinho);
    $stmt->execute();
...
{
    $sql = "INSERT INTO pedido_item (cod_pedido, cod_produto, valor_item, quantidade) 
    VALUES valor_item FROM produto WHERE cod_produto =?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iii", $cod_pedido, $cod_produto, $quantidade);
...
{
    $sql = "SELECT p.cod_pedido, c.nome_cliente FROM pedido p 
